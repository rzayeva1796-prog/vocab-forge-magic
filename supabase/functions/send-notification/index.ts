...
serve(async (req) => {
  ...
  try {
    ...
    // Store notification in pending_notifications - this is the primary delivery method
    ...
    console.log('Notification stored successfully for user:', targetUserId);
    console.log('=== NOTIFICATION COMPLETE ===');

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Notification stored successfully. User will receive it via polling.',
        stored: true
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
